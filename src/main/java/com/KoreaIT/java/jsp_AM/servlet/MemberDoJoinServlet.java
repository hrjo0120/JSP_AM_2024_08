		...
		try {
			...
			if (isJoinableLoginId == false) {
				response.getWriter().append(String
						.format("<script>alert('%s는 이미 사용중인 아이디입니다.'); location.replace('../member/join');</script>", loginId));
				return;
			}
			...
			response.getWriter().append(
					String.format("<script>alert('%d번 회원이 가입 되었습니다'); location.replace('../article/list');</script>", id));
